...
def create_practice_geographical_list(
    ...
    csv_paths = [str(p) for p in gp_list_csv_list]
    with duckdb.connect() as con:
        con.sql(f"""
        COPY (
            SELECT
                distinct l.* exclude(NAT_ONS_CODE, NAT_CODE, COUNTRY, NAT_COUNTRY),
            ...
            FROM read_csv({csv_paths}, sample_size = -1, union_by_name = true) l
...
def main():
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        address_csv = Practice_location_info / "epraccur.csv"
        practice_geo_list = create_practice_geographical_list(
            address_csv, raw_qof_geo_map_path_list, tmpdirname
        )
